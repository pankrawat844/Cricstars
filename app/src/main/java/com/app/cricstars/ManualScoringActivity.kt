...
import kotlinx.android.synthetic.main.fr_leaderboard_tabs.*
...
class ManualScoringActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tv_bowler_name?.setOnClickListener(this)
        tvBats1?.setTextColor(Color.YELLOW)
        tvBats2?.setTextColor(Color.WHITE)

        toss.text = intent.getStringExtra("toss_won") + " won the toss and elected to " + intent.getStringExtra(
                "choose"
            )
        fullScoreCard.setOnClickListener {
            val int = Intent(this@ManualScoringActivity, MatchDetailsActivity::class.java)
            int.putExtra("match_id",intent.getStringExtra("match_id"))
            startActivity(int)
        }
...
//                arrayRun.add(new RunsModel("0", "nb"));
            ...
            R.id.tv_bowler_name -> alertBoxBowler(
                "Are your sure?",
                "Are your sure to change bowler manually ?",
                1
            )
        }
...
//                if (i == 0) {
                    val temp2Runs=tvBatsRun2!!.text.toString()
                    val tempBatsname=tvBats2!!.text.toString()

                    tvBats2!!.text=tvBats1!!.text.toString()
                    tvBatsRun2!!.text=tvBatsRun1!!.text.toString()
                    tvBats1!!.text=tempBatsname
                    tvBatsRun1!!.text=temp2Runs
//                } else {
//                    tvBats1!!.setTextColor(Color.WHITE)
//                    tvBats2!!.setTextColor(Color.YELLOW)
//                }
    ...
    fun alertBoxBowler(title: String?, msg: String?, i: Int) {
        val alertDialog: AlertDialog = AlertDialog.Builder(this).create()
        alertDialog.setTitle(title)
        alertDialog.setMessage(msg)
        alertDialog.setButton(AlertDialog.BUTTON_NEUTRAL, "OK",
        ...
        alertDialog.setButton(AlertDialog.BUTTON_NEUTRAL, "OK",
            DialogInterface.OnClickListener { dialog, which ->
                if(inningNo==1) {
                    ...
                    team2 = team1
                    team1 = temp1
                    Log.e("team1",team1List.toString())
                    Log.e("team2",team2List.toString())
    ...
    fun showBottomSheet(viewId: Int?, word: String?) {
        ...
        var strikeChng=0
                ...
                if (word == "W")
                    dialogBox(word, bowlToCount, i,strikeChng)
                else {
                    if (word == "NB" || word == "WD")
                        extraRun =  1
                    else {
...
//                                strikeBatsman = batsmanId.t10_team_member_id
    ...
    fun dialogBox(word: String, bowlToCount: String, i: Int,strikeChange:Int) {
        ...
        val set=HashSet<String>(team2listString);
        team2listString.clear()
        team2listString.addAll(set)
        team2listString.add(0, "Helping fielder")
        ...
        select_wicket_type.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                ...
                if(select_wicket_type.selectedItem.toString()=="LBW" || select_wicket_type.selectedItem.toString()=="Hit Wicket" || select_wicket_type.selectedItem.toString()=="Bowled"   )
                    select_wicket_type_helper.visibility=View.GONE
                else
                    select_wicket_type_helper.visibility=View.VISIBLE
            }
        ...
        val set1=HashSet<String>(team1listString);
        team1listString.clear()
        team1listString.addAll(set1)
        ...
        submit.setOnClickListener {
            ...
            if(select_wicket_type.selectedItem.toString()=="Catch Out" || select_wicket_type.selectedItem.toString()=="Stumping"    )
            {
            if (select_wicket_type_helper.selectedItem.toString() == "Helping fielder") {
                Toast.makeText(
                    this@ManualScoringActivity,
                    "Please select helping fielder",
                    Toast.LENGTH_SHORT
                ).show()
                return@setOnClickListener
            }
    ...
    fun dialogBoxNewInning() {
        ...
        val set=HashSet<String>(batsmanListName);
        batsmanListName.clear()
        batsmanListName.addAll(set)
        ...
        val set1=HashSet<String>(batsmanListName2);
        batsmanListName2.clear()
        batsmanListName2.addAll(set1)
        batsmanListName2.add(0,"Select Batsman 2")
        ...
        val bowlerset=HashSet<String>(bowlerList)
        bowlerList.clear()
        bowlerList.addAll(bowlerset)
        ...
        submit.setOnClickListener {
            ...
            tvBolwerStats?.text = "0-0-0-0" //OMRWE
             tvBats1!!.setTextColor(Color.YELLOW)
            tvBats2!!.setTextColor(Color.WHITE)
            dialog.dismiss()
...
//        }
        ...
        val set=HashSet<String>(bowlerListString);
        bowlerListString.clear()
        bowlerListString.addAll(set)
        bowlerListString.add(0, "Select Bowler")
        ...
        submit.setOnClickListener {
           ...
           tvBolwerStats?.text ="0-0-0-0"
            var bolwerID=""
            team2List.forEach {
                if(it.t10_team_member_name==tv_bowler_name.text.toString())
                    bolwerID=it.t10_team_member_id
            }
            getCurrentBowler(intent.getStringExtra("match_id"),bolwerID)
            dialog.dismiss()
        }

    }

    class OverDialog(var c: Activity) : Dialog(c) {
        var d: Dialog? = null
...
//                                    R.id.tv_lb, R.id.tv_bye -> {
                                if(extraType=="WD" || extraType=="NB")
                                        arrayRun.add(RunsModel(run, extraType))
                                else
                    ...
                    override fun onResponse(
                        ...
                        if (response.isSuccessful) {
                            ...
                            if (data?.success!!) {
                                ...
                                tvBatsRun1!!.text= "${data!!.data!!.strikeBatsmanRun}(${data!!.data!!.strikeBatsmanBall})"
                                tvBats1!!.text = "${data!!.data!!.strikeBatsman}"

                                tvBats2!!.text= "${data!!.data!!.nonstrikeBatsman}"

                                tvBatsRun2!!.text =
                                    "${data!!.data!!.nonstrikeBatsmanRun}(${data!!.data!!.nonstrikeBatsmanBall})"

                                if (data!!.data!!.inning2Teamscore!= 0) {
                                    toss!!.text=data!!.data!!.toss_string
                                    battingTeam.text=data!!.data!!.bowling_team_name
                                    tvScoreWkt!!.text = data!!.data!!.inning2wicket.toString()
                                    tvScoreRun!!.text = data!!.data!!.inning2Teamscore.toString()+"/"
                                    tv_bowler_name!!.text=data!!.data!!.currentBowler
                                    tvBolwerStats?.text =data!!.data!!.currentBowlerOver.toString()+"-"+data!!.data!!.currentBowlerRuns+"-"+data!!.data!!.currentBowlerMedian+"-"+data!!.data!!.currentBowlerWicket
                                        //OMRWE
                                    if(data!!.data!!.inningTwoCompleted){
//                                        dialogBoxNewInning()

                                    }else {
                                        if (data!!.data!!.currentBowlerOver.toString()
                                                .substring(data!!.data!!.currentBowlerOver.toString().length - 1)
                                                .equals("6")
                                        ) {
                                            ...
                                            tvBats1!!.text= "${data!!.data!!.nonstrikeBatsman}"

                                            tvBatsRun1!!.text =
                                                "${data!!.data!!.nonstrikeBatsmanRun}(${data!!.data!!.nonstrikeBatsmanBall})"

                                            tvBatsRun2!!.text= "${data!!.data!!.strikeBatsmanRun}(${data!!.data!!.strikeBatsmanBall})"
                                            tvBats2!!.text = "${data!!.data!!.strikeBatsman}"

                                        }

                                    }
//                                Toast.makeText(this@ManualScoringActivity, "${data.data.toString()}", Toast.LENGTH_SHORT)
//                                    .show()
                                    ...
                                    toss!!.text = data!!.data!!.toss_string
                                    battingTeam.text = data!!.data!!.bating_team_name
                                    tvScoreWkt!!.text = data!!.data!!.inning1wicket.toString()
                                    tvScoreRun!!.text =
                                        data!!.data!!.inning1Teamscore.toString() + "/"
                                    tv_bowler_name!!.text = data!!.data!!.currentBowler
                                    tvBolwerStats?.text =
                                        data!!.data!!.currentBowlerOver.toString() + "-" + data!!.data!!.currentBowlerRuns + "-" + data!!.data!!.currentBowlerMedian + "-" + data!!.data!!.currentBowlerWicket
                                    //OMRWE

                                    if(data!!.data!!.inningOneCompleted){
                                        inningNo=2
                                        val temp = team2List
                                        val temp1 = team2
                                        team2List = team1List
    ...
    private fun getCurrentBowler(matchID:String,bowlerID:String) {
        loader.visibility = View.VISIBLE
        CoroutineScope(Dispatchers.Main).launch {
            val retrofitMethod = ApiBuilder.getRetrofitInstance(this@ManualScoringActivity)?.create(
                RetrofitMethods::class.java
            )
            retrofitMethod?.getCurrentBowler(
              matchID,
                bowlerID
            )?.enqueue(
                object : Callback<CurrentBowler?> {
                    override fun onResponse(
                        call: Call<CurrentBowler?>,
                        response: Response<CurrentBowler?>
                    ) {
                        loader.visibility = View.GONE
                        if (response.isSuccessful) {
                            val data = response.body()
                    ...
                    override fun onFailure(call: Call<CurrentBowler?>, t: Throwable) {
                        loader.visibility = View.GONE
                        Toast.makeText(this@ManualScoringActivity, t.message, Toast.LENGTH_SHORT)
                            .show()
    ...
    private fun undoScore() {
        loader.visibility = View.VISIBLE
        CoroutineScope(Dispatchers.Main).launch {
            val retrofitMethod = ApiBuilder.getRetrofitInstance(this@ManualScoringActivity)?.create(
                RetrofitMethods::class.java
            )
            retrofitMethod?.undoScore(
                intent.getStringExtra("match_id")
            )?.enqueue(
                object : Callback<GenericResponse?> {
                    override fun onResponse(
