...
class RecyclerTournamentMatchesAdapter(var context: Activity, var darkOrLight: Int, var data:MatchDetail?) :
    RecyclerView.Adapter<RecyclerTournamentMatchesAdapter.MyViewHolder?>() {
    //declare interface
...
//         holder.tv_team1_overs?.setText(data?.data?.get(position)?.match_details.)
                ...
                if (data?.data?.get(position)?.match_details?.t10_toss_won == data?.data?.get(position)?.t10_team_A_id) {
                    if (data?.data?.get(position)?.teamAname != null)
                        holder.tv_status?.setText(data?.data?.get(position)?.teamAname + " won the toss")
                    else
                        holder.tv_status?.setText(" won the toss")
                } else {
                    if (data?.data?.get(position)?.teamBname != null)
                        holder.tv_status?.setText(data?.data?.get(position)?.teamBname + " won the toss")
                    else
                        holder.tv_status?.setText(" won the toss")
