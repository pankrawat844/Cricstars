...
class RecyclerBattingAdapter(var context: Activity, var bowlOrBat: Int,var data:List<MatchScoreDetail.Data.Inning1Score>) :
    RecyclerView.Adapter<RecyclerBattingAdapter.MyViewHolder?>() {
    //declare interface
   ...
   override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
       ...
       if(model.bowlerName.isEmpty())
           ...
           if(model.wicketHelperName==null)
               holder.tvOutBy.text =
                   model.wicket_type + " n b " + model.bowlerName
           else
           holder.tvOutBy.text =
               model.wicket_type + " " + model.wicketHelperName + " b " + model.bowlerName
       }
